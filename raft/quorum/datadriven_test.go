...
func TestDataDriven(t *testing.T) {
	datadriven.Walk(t, "testdata", func(t *testing.T, path string) {
		datadriven.RunTest(t, path, func(d *datadriven.TestData) string {
			...
			// to 5 and that for 4 to 7 (and no others).
			//
			...
			// Helper that returns an IndexLookuper which has the specified indexes
			// mapped to the right IDs.
			makeLookuper := func(idxs, ids, idsj []uint64) mapLookuper {
				l := mapLookuper{}
			...
			case "committed":
				...
				if !joint {
					...
					overlay := func(c MajorityConfig, l IndexLookuper, id, idx uint64) IndexLookuper {
						ll := mapLookuper{}
						for iid := range c {
							if iid == id {
								ll[iid] = idx
							} else if idx, ok := l.Index(iid); ok {
								ll[iid] = idx
							}
						}
						return ll
					...
					for id := range c {
						...
						} else if cr.Maybe < idx && idx < math.MaxUint64 {
							// If an index has no chance of becoming a committed
							// index, we should be able to increase it without
							// changing the result.
							lo := overlay(c, l, id, idx+1)
							if cro := c.CommittedIndex(lo); cro != cr {
								fmt.Fprintf(&buf, "%s <-- overlaying %d->%d", cro, id, idx)
			...
			case "vote":
				...
				if !joint {
					...
					r := c.VoteResult(l)
					// Test via the alternative impl of CommittedIndex as well.
					if ar := voteResultVia(c, l, alternativeMajorityCommittedIndex); ar != r {
						fmt.Fprintf(&buf, "%v <-- via alternative computation\n", ar)
					}
					fmt.Fprintf(&buf, "%v\n", r)
				} else {
